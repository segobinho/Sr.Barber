...
function Clientes() {
    ...
    useEffect(() => {
        ...
        if (userData) {
            axios.get('http://localhost:8800/se', {
                params: {
    ...
    const validateFields = () => {
        if (!values.nome || !values.preco || (user.cargo === 'admin' && !values.id_barbearia)) {
    ...
    const handleClienteAdd = () => {
        ...
        axios.post('http://localhost:8800/service', {
            ...
            const successMessage = response.data.message || 'Serviço adicionado com sucesso!';
            const newService = response.data.service;

            if (successMessage === "Já existe um serviço ativo com esse nome, preço e barbearia.") {
                toast.info(successMessage);
            } else {
                setClientes([...clientes, newService]); // Atualiza o estado com o novo serviço
                setClientesFiltrados([...clientesFiltrados, newService]); // Caso você use um filtro separado
                setValues({}); // Reseta os campos do formulário
                setShowAddClienteForm(false); // Fecha o formulário
    ...
    const handleSaveEdit = () => {
        axios.put(`http://localhost:8800/service/${selectedCliente.id_servico}`, values)
            .then((response) => {
                const updatedService = response.data.service; // Dados retornados pelo backend
                setClientes(clientes.map(cliente =>
                    cliente.id_servico === selectedCliente.id_servico ? updatedService : cliente
                ));
                setIsEditing(false);
                setSelectedCliente(updatedService); // Atualiza os dados do cliente selecionado
                setShowAddClienteForm(false);
                toast.success(response.data.message || 'Cliente editado com sucesso!'); // Usa a mensagem do backend, se disponível
            })
            .catch((error) => {
                console.error("Erro ao salvar edição:", error);
                console.log(values);
                const errorMessage = error.response?.data?.message || 'Erro ao editar cliente.';
                toast.error(errorMessage); // Exibe a mensagem de erro no toast
            });
    };
    ...
    const handleClienteDelete = (clienteId) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`http://localhost:8800/service/${clienteId}`)
                    .then((response) => {
                        setClientes(clientes.filter(cliente => cliente.id_servico !== clienteId));
                        setSelectedCliente(null);
                        const successMessage = response.data.message || 'Serviço deletado com sucesso!';
                        toast.success(successMessage);
                    })
                    .catch((error) => {
                        console.error("Erro ao deletar Serviço:", error);
                        const errorMessage = error.response?.data?.message || 'Erro ao deletar Serviço.';
        ...
        <div>
            ...
            <Main>
                ...
                <div className="container">
                    <div className="bloco">
                        <div className="title" >
                            <h1 onClick={handleTitleClick}>Serviços</h1>
                            <AddCircleOutline className="button1" onClick={handleClienteAddForm} />
                        </div>
                        <hr />
                        <div>
                            <FiltroBusca itens={clientes} onFiltrar={setClientesFiltrados} placeholder={"Buscar Serviço"} />
                            {clientesFiltrados.map(cliente => (
                                ...
                                >
                                    <CardContent>
                                        ...
                                        <Typography variant="subtitle2" className="clientep" sx={{ color: 'yellow', fontSize: '0.8rem' }}>
                                            <strong>Preço:</strong> {parseFloat(cliente.preco).toFixed(2)}
                                        </Typography>
                                    </CardContent>
                                </Card>
                            ))}
                        ...
                        <div className="container1">
                            {showAddClienteForm && (
                                <div className="container2">
                                    <h2 className="cliente-title3">{isEditing ? "Editar Serviço" : "Adicionar Serviço"}</h2>
                                    <TextField
                                        label="Nome"
                                        type="text"
                                        name="nome"
                            ...
                            {!showAddClienteForm && !isEditing && selectedCliente && (
                                <div className="container2">
                                    <h2 className="cliente-title3">Detalhes do Serviço</h2>
                                    <div className="cliente-info3">
                                        <p><strong>Nome:</strong> {selectedCliente.nome}</p>
                                        <p> <strong>Preço:</strong> {parseFloat(selectedCliente.preco).toFixed(2)}</p>
                                    ...
                                    <div className="icon-container">
                                        ...
                                        <CiTrash className="icon-delete" title="Deletar Cliente" onClick={() => {
                                            console.log('Tentando deletar cliente com ID:', selectedCliente.id_servico);
                                            handleClienteDelete(selectedCliente.id_servico);
                                        }} />
                            ...
                            {!showAddClienteForm && !isEditing && !selectedCliente && (
                                ...
                                <>
                                    <p className="clique">Clique em um Serviço para ver os detalhes</p>
                                </>
                            )}
                        </div>
                    </div>
